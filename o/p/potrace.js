H.Potrace=(function(){
	...
	Bitmap.prototype.copy=function(){
		...
		for(let i=0;i<this.size;i++)bm.data[i]=this.data[i];
		return bm;
	};
	function Path(){
		this.pt=[];
		this.curve={};
		this.area=this.len=0;
		this.maxX=this.maxY=-1;
		this.minX=this.minY=100000;
	}
	function Curve(n){
		this.n=n;
	...
	let bm=null,pathlist=[],callback,info={
		isReady:false,
		turnpolicy:'minority',
		turdsize:2,optcurve:true,
		alphamax:1,opttolerance:0.2
	};
	function load(c){
		if(info.isReady)clear();
		let ctx=c.getContext('2d');
		bm=new Bitmap(c.width,c.height);
		let imgdataobj=ctx.getImageData(0,0,bm.w,bm.h);
		let l=imgdataobj.data.length;
		for(let i=0,j=0;i<l;i+=4,j++){
			let color=0.2126*imgdataobj.data[i]+0.7153*imgdataobj.data[i+1]+0.0721*imgdataobj.data[i+2];
			bm.data[j]=(color<128?1:0);
		}
		info.isReady=true;
	}
	function config(obj){
		for(let key in obj)if(obj.hasOwnProperty(key))info[key]=obj[key];
	}
	function bmToPathlist(){
		let bm1=bm.copy(),currentPoint=new Point(0,0),path;
		...
		function findPath(point){
			let path=new Path(),x=point.x,y=point.y,dirx=0,diry=1,tmp;
		...
		function xorPath(path){
			let y1=path.pt[0].y,len=path.len;
			for(let i=1;i<len;i++){
				let x=path.pt[i].x;
				let y=path.pt[i].y;
				if(y!==y1){
	...
	function processPath(){
		...
		function calcLon(path){
			let n=path.len,pt=path.pt,dir,pivk=new Array(n),nc=new Array(n),ct=new Array(4);
			path.lon=new Array(n);
			let constraint=[new Point(),new Point()],cur=new Point(),off=new Point(),dk=new Point(),foundk;
		...
		function bestPolygon(path){
			function penalty3(path,i,j){
				...
				let x,y,xy,x2,y2,k,a,b,c,s,px,py,ex,ey,r=0;
				if(j>=n){
					j-=n;
					r=1;
				}
			...
			let i,j,m,k,n=path.len,pen=new Array(n+1),prev=new Array(n+1),clip0=new Array(n),clip1=new Array(n+1),seg0=new Array(n+1),seg1=new Array(n+1),thispen,best,c;
			for(i=0;i<n;i++){
		...
		function adjustVertices(path){
			function pointslope(path,i,j,ctr,dir){
				let n=path.len,sums=path.sums,x,y,x2,xy,y2,k,a,b,c,lambda2,l,r=0;
				while(j>=n){
					j-=n;
					r+=1;
				}
			...
			let m=path.m,po=path.po,n=path.len,pt=path.pt,x0=path.x0,y0=path.y0,ctr=new Array(m),dir=new Array(m),q=new Array(m),v=new Array(3),d,i,j,k,l,s=new Point();
		...
		function optiCurve(path){
			...
			function opti_penalty(path,i,j,res,opttolerance,convc,areac){
				let m=path.curve.n,curve=path.curve,vertex=curve.vertex,k,k1,k2,conv,i1,area,alpha,d,d1,d2,p0,p1,p2,p3,pt,A,R,A1,A2,A3,A4,s,t;
				if(i==j)return 1;
				k=i;
				i1=mod(i+1,m);
				k1=mod(k+1,m);
			...
			let curve=path.curve,m=curve.n,vert=curve.vertex,pt=new Array(m+1),pen=new Array(m+1),len=new Array(m+1),opt=new Array(m+1),om,i,j,r,o=new Opti(),p0,i1,area,alpha,ocurve,s,t;
		...
		for(let i=0;i<pathlist.length;i++){
			...
			smooth(path);
			if(info.optcurve)optiCurve(path);
		}
	}
	function process(c){
		if(c)callback=c;
	...
	function get_svg(size,fillc='black',only_path=false,opt_type=null){
		function path(curve){
			const fix=n=>parseFloat(n.toFixed(3));
			function bezier(i){
				let b='C'+fix(curve.c[i*3+0].x*size)+' '+fix(curve.c[i*3+0].y*size)+',';
				b+=fix(curve.c[i*3+1].x*size)+' '+fix(curve.c[i*3+1].y*size)+',';
				b+=fix(curve.c[i*3+2].x*size)+' '+fix(curve.c[i*3+2].y*size)+' ';
				return b;
			}
			function segment(i){
				let s='L '+fix(curve.c[i*3+1].x*size)+' '+fix(curve.c[i*3+1].y*size)+' ';
				s+=fix(curve.c[i*3+2].x*size)+' '+fix(curve.c[i*3+2].y*size)+' ';
				return s;
			}
			let n=curve.n,i;
			let p='M'+fix(curve.c[(n-1)*3+2].x*size)+' '+fix(curve.c[(n-1)*3+2].y*size)+' ';
			for(i=0;i<n;i++){
		...
		let w=bm.w*size,h=bm.h*size,len=pathlist.length,c,i,strokec,fillrule;
		let svg=only_path?'':`<svg viewBox='0 0 ${w} ${h}' xmlns='http://www.w3.org/2000/svg'>`;
		svg+=`<path d='`;
		for(i=0;i<len;i++){
			c=pathlist[i].curve;
			svg+=path(c);
