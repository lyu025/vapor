class Jssj{
	static #h={
		'x-app-id':'bVBF4FyRTn5NJF5n','x-version':'1.0','x-token':'',Withcredentials:true,
	...
	static async filters(is_run,o_parser){
		...
		return await fetch(Vapor.link(url),{headers:Jssj.#h,credentials:'include'}).then(_=>_.json()).then(({data})=>{
			if(!is_run())return
			o_parser(data.map(({id,name:title})=>({id,title})))
		}).catch(_=>toast.error('网络请求失败!'))
	}
	static async list(is_run,o_parser,tid){
		...
		return await fetch(Vapor.link(url),{headers:Jssj.#h,credentials:'include'}).then(_=>_.json()).then(({data})=>{
			...
			const o=data.map(({id,time,data:{content:title,pic:img,source:from},important:red})=>{
				return title?{x:id,ni:'1',time,title,img:img||'',from:from||'',red:!!red}:null
			}).filter(Boolean)
