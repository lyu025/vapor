class Radio extends Page{
	static #o=null;
	static open(){
		if (!this.#o)this.#o=new Radio();
		return this.#o
	}
	define(){
		this.w={}
		this.wm={zgzs:'中国之声',hqzx:'环球资讯',jtgb:'交通广播',jjzs:'经济之声'};
		this.pholder='data:image/svg+xml;base64,'
		this.loader=`<svg class='radio_loader' viewBox='0 0 50 50'><path d='M25,5 a20,20 0 1,1 -20,20' stroke='var(--fg)' stroke-width='1' fill='none' stroke-linecap='round'><animate attributeName='stroke-dasharray' values='10,60;60,10;10,60' dur='1.5s' repeatCount='indefinite'/><animateTransform attributeName='transform' type='rotate' from='0 25 25' to='360 25 25' dur='1s' repeatCount='indefinite'/></path></svg>`
	}
	constructor(){
		if(Radio.#o)return
		super('radio')
	}
	nodes(){
		return[
			this.N('card',{id:'china',cc:'china'},
				this.N('h2',this.N('svg',{path:'china'}),'中国广播',
					this.N('svg',{id:'wb_back',path:'back',h:true,click:'App.pages.radio.china_back(this)'}),
				),
				this.N('div',{id:'wb_home',T:'A',c:'grid',s:'--gc:2'},
					...Object.keys(this.wm).map(_=>this.N('div',{X:_,N:this.wm[_],click:`App.pages.radio.china_filters("${_}",this)`},this.N('svg',{path:_})))
				),
				this.N('div',{id:'wb_filters',T:'F',h:true}),
				this.N('div',{id:'wb_list',T:'W',h:true,c:'grid',s:'--gc:1',more:'App.pages.radio.china_list'}),
				this.N('div',{id:'wb_info',T:'O',h:true,c:'row'}),
			),
		]
	}
	styles(){
		return[
			'ᝰ>[cc="china"]>h2>svg:nth-child(2){margin-left:auto;width:22px;height:22px}',
			'ᝰ>[cc="china"]>[T="A"]>*{aspect-ratio:22/9;padding:0 3rem}',
			'ᝰ>[cc="china"]>[T="A"]>div>svg{width:100%;height:100%;object-fit:contain}',
			
			'ᝰ>[cc="china"]>[T="F"]{position:sticky;top:36px;z-index:100000;min-height:20px;background:var(--bg);display:flex;flex-direction:column}',
			'ᝰ>[cc="china"]>[T="F"]:empty:after{content:"正在加载筛选项，请耐心等待一下 ...";display:block;width:100%;position:absolute;top:50%;left:0;z-index:10;transform:translateY(-50%);font-size:.8rem;text-align:center}',
			'ᝰ>[cc="china"]>[T="F"]>div{display:block;white-space:nowrap;min-width:100%;overflow-x:auto;overflow-y:hidden}',
			'ᝰ>[cc="china"]>[T="F"]>div::-webkit-scrollbar{height:4px}',
			'ᝰ>[cc="china"]>[T="F"]>div>*{border-bottom:2px solid rgba(0,0,0,0);display:inline-block;width:auto;padding:0 1rem;line-height:24px;font-size:.9rem}',
			'ᝰ>[cc="china"]>[T="F"]>div>[active]{border-bottom-color:var(--h-bd);font-size:1rem}',
			
			'ᝰ>[cc="china"]>[T="W"]>div{display:flex;flex-direction:column;height:auto;aspect-ratio:unset;border:.02rem solid var(--h-bd);box-shadow:4px 2px rgba(100,100,100,.3)}',
			'ᝰ>[cc="china"]>[T="W"]>div[in="1"]{border:.02rem solid var(--success);box-shadow:4px 2px var(--success)}',
			'ᝰ>[cc="china"]>[T="W"]>div>div:first-child{font-size:1.1rem;padding:8px 0 2px 3px;line-height:1.2;background:rgba(200,200,200,.1)}',
			'ᝰ>[cc="china"]>[T="W"]>div>div:first-child>svg{display:block;float:left;width:28px;height:16px;object-fit:contain;margin-right:5px}',
			'ᝰ>[cc="china"]>[T="W"]>div>brief{display:block;border-radius:2px;background:rgba(150,150,150,.1);color:var(--success);font-size:.8rem;line-height:1.05;padding:4px}',
			'ᝰ>[cc="china"]>[T="W"]>div>time{display:block;color:var(--ph);font-size:.9rem;line-height:1.05;padding:4px}',
			'ᝰ>[cc="china"]>[T="W"]>div>.imgs{height:90px;overflow:hidden}',
			'ᝰ>[cc="china"]>[T="W"]>div>.imgs>img{display:block;float:left;height:90px;object-fit:contain}',
			
			'ᝰ>[cc="china"]>[T="O"]{display:flex;flex-direction:column;min-height:30vh;padding:1rem}',
			'ᝰ>[cc="china"]>[T="O"]>svg{width:24%;margin:1rem auto;object-fit:contain;flex:1}',
			'ᝰ>[cc="china"]>[T="O"]>title{display:block;width:100%;line-height:1.2;margin-top:6px;font-size:1.1rem;font-weight:bold}',
			'ᝰ>[cc="china"]>[T="O"]>time{display:block;width:100%;margin-top:6px;color:var(--ph)}',
			'ᝰ>[cc="china"]>[T="O"]>div{padding:8px;background:rgba(150,150,150,.2);border-radius:3px;margin-top:6px;font-size:.9rem}',
			'ᝰ>[cc="china"]>[T="O"]>div>p{line-height:1.3;font-size:.9rem;margin:10px 0 0 0}',
			'ᝰ>[cc="china"]>[T="O"]>div>strong{display:block;line-height:1.6;font-size:1.1rem;margin-top:14px;font-weight:bold}',
			'ᝰ>[cc="china"]>[T="O"] img{display:block;width:100%;object-fit:contain;margin-top:6px}',
		]
	}
	china_back(e){
		this.E('china').d_attr('wait')
		if(!this.E('wb_info').h_attr('hide')){
			const id=this.E('wb_info').g_attr('i')
			this.E('wb_info').s_attr('hide').d_attr('i').html('')
			this.E('wb_filters').d_attr('hide')
			this.E('wb_list').d_attr('hide').node('[i="'+id+'"]').scrollIntoView({block:'center'})
			return
		}
		this.E('china').node('h2').childNodes[1].textContent='媒体源'
		this.E('wb_filters').s_attr('hide').html('')
		this.E('wb_info').s_attr('hide').html('')
		this.E('wb_list').s_attr('hide').html('')
		this.E('wb_home').d_attr('hide')
		e.s_attr('hide')
		this.w={}
	}
	async china_filters(X){
		const ct=eval(`typeof ${fucase(X)}`)
		if(ct=='undefined'){
			this.script(`/addons/${X}.js`,`App.pages.radio.china_filters("${X}")`)
			return
		}
		this.w={X}
		this.E('wb_back').d_attr('hide')
		this.E('wb_home').s_attr('hide')
		this.E('wb_list').d_attr('hide').html('')
		this.E('wb_info').s_attr('hide').html('')
		this.E('wb_filters').html('').d_attr('hide')
		this.E('china').node('h2').childNodes[1].textContent=this.wm[X]
		eval(fucase(X)).filters(()=>this.w.X==X,o=>{
			const $=o.map(({id,title})=>this.N('div',{t:id,n:title,click:'App.pages.radio.china_list(this)'},title))
			this.E('wb_filters').append(this.N('div',...$))
			this.E('wb_filters').node('div>[t]').click()
		})
	}
	async china_list(e,p=1){
		if(p=='..')return
		if(e){
			this.w.T=e.g_attr('t')
			this.E('wb_list').s_attr({p:1}).html('')
			e.parentNode.childNodes.forEach(_=>_[_==e?'s_attr':'d_attr']('active'))
		}
		this.E('china').s_attr('wait')
		const T=this.w.T,next=await eval(fucase(this.w.X)).list(()=>this.w.T==T,async o=>{
			let oo
			if(this.w.X=='fxb'){
				const x=[]
				o.forEach(({title,brief})=>x.push(title,brief))
				oo=await this.trans(x)
			}
			const sid=(this.cache(`radio_${this.w.X}_${this.w.T}`)||'')+','
			const $=o.map(({x,id,ni,url,title,brief,img,time,red},i)=>{
				const tt=this.N('div',red?this.N('svg',{path:'important'}):'')
				tt.innerHTML+=oo?oo[i*2]:title
				const n=sid===','?true:!(sid+',').includes(`,${x},`)
				return this.N('div',{i:id,x,in:n?'1':'0',ni,click:'App.pages.radio.china_info(this)'},
					tt,brief?this.N('brief',oo?oo[i*2+1]:brief):'',
					img?this.N('div',{c:'imgs'},this.N('img',{src:this.pholder,ss:this.link(img)})):'',
					time?this.N('time',time):'',
				)
			})
			this.E('wb_list').append(...$)
		},this.w.T,p)
		if(next=='✘')return
		this.E('china').d_attr('wait')
		this.E('wb_list').s_attr({p:next})
	}
	async china_info(e){
		const x=e.g_attr('x').trim()
		const xs=this.cache(`radio_${this.w.X}_${this.w.T}`)||''
		this.cache(`radio_${this.w.X}_${this.w.T}`,`${xs},${x}`)
		e.d_attr('in')
		if(e.g_attr('ni')=='1')return
		const id=e.g_attr('i').trim(),T=this.w.T
		this.E('wb_filters').s_attr('hide')
		this.E('wb_list').s_attr('hide')
		this.E('wb_home').s_attr('hide')
		this.E('wb_info').html(this.loader).d_attr('hide').s_attr({i:id})
		
		eval(fucase(this.w.X)).info(()=>this.w.T==T,async({title,time,box})=>{
			let oo
			if(this.w.X=='fxb'){
				oo=await this.trans([title,...box])
				for(let i in oo)if(i>0){
					const v=oo[i].trim()
					if(!v.startsWith('<'))oo[i]=(i==1?'<p>':'<p>&emsp;&emsp;')+v+'</p>'
					else if(!v.split('>')[1].startsWith('&emsp;')){
						const [vf,...ov]=v.split('>')
						oo[i]=vf+(i==1?'>':'>&emsp;&emsp;')+ov.join('>')
					}
				}
			}
			this.E('wb_info').append(
				this.N('title',oo?oo.shift():title),
				this.N('time',time),
				this.N('div'),
			)
			this.E('wb_info').node('title').scrollIntoView({block:'start'})
			this.E('wb_info').node('.radio_loader')?.remove()
			this.E('wb_info').node('div').innerHTML=(oo||box).join('').trim()
		},id)
	}
}