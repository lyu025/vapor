class Img extends Page{
	...
	styles(){
		return[
			'ᝰ>[cc="icons"]>h2>svg{margin-left:auto;width:22px;height:22px}',
			'ᝰ>[cc="icons"]>.grid>div{aspect-ratio:9/12;display:flex;flex-direction:column;position:relative}',
			'ᝰ>[cc="icons"]>.grid>div>i{display:block;width:auto;line-height:.8rem;font-size:.8rem;position:absolute;top:4px;left:4px;border-bottom:1px solid var(--h-bg);border-radius:2px}',
			'ᝰ>[cc="icons"]>.grid>div>svg{flex:1;display:block;width:80%;object-fit:contain;margin:0 auto}',
			'ᝰ>[cc="icons"]>.grid>div>div{padding:0 1rem;height:16px;font-size:1.1rem;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
	...
	icons_search(is_restart=false){
		...
		fetch(url,{method:'POST',body:body.toString(),headers:{'x-up':'true','Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}}).then(_=>_.json()).then(_=>{
